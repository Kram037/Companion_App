...
import { combatMonsterSchema, parseArray, parseNullable, sessionSchema } from '../schemas';
...
export async function fetchCombatSnapshot(campagnaId: Id, sessioneId: Id): Promise<CombatSnapshot> {
  ...
  const [sessionResult, rolls, mostri, personaggi] = await Promise.all([
    client.from('sessioni').select('*').eq('id', sessioneId).maybeSingle(),
  ...
  return { sessione: parseNullable(sessionSchema, sessionResult.data), tiri: rolls, mostri, personaggi };
}
...
async function fetchInitiativeRolls(sessioneId: Id): Promise<InitiativeRoll[]> {
  ...
  if (error) {
    ({ data, error } = await client.from('richieste_tiro_iniziativa').select('*').eq('sessione_id', sessioneId));
  }
  throwIfSupabaseError(error);
